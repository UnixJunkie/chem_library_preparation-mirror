...
SEED 3 modification => Sybyl atom types are changed to CGenFF atom types
"""
import sys, os
	...
	if start==True:
		...
		if "@<TRIPOS>BOND" in line:
			...
			for key, value in LP_dict.iteritems():
				...
				out+=("%7s %-8s%10.4f%10.4f%10.4f %-7s%5s %-11s%7.4f\n") %(atom_indice, key, coor[0], coor[1], coor[2], "LPH", "0", "0", 0.0500)
		...
		if whereami_mol2==False:
			...
			if len(line) > 2:
				...
				out+=("%7s %-8s%10.4f%10.4f%10.4f %-7s%5s %-11s%7.4f\n") %(int(atom_indice), line.split()[1], float(line.split()[2]), float(line.split()[3]), float(line.split()[4]), line.split()[5], line.split()[6], line.split()[7], float(dict_charges[atom_indice]))	
				atom_indice+=1
			else:
				out+=line
